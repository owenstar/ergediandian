...
@property (nonatomic, strong) TBSlider       *playSlider;  //滑竿
@property (nonatomic, strong) UIButton       *stopButton;//播放暂停按钮
@property (nonatomic, strong) UIButton       *sigleCircleButton;//单曲循环按钮
...
- (void)playWithUrl:(NSURL *)url showView:(UIView *)showView
{
    ...
    if(!self.currentPlayerLayer){
        ...
        self.currentPlayerLayer.frame = _showView.bounds;
...
- (void)seekToTime:(CGFloat)seconds
{
    ...
    [self.player seekToTime:CMTimeMakeWithSeconds(seconds, NSEC_PER_SEC) completionHandler:^(BOOL finished) {
        ...
        if (!weakSelf.currentPlayerItem.isPlaybackLikelyToKeepUp) {
            weakSelf.state = TBPlayerStateBuffering;
            [[XCHudHelper sharedInstance] showHudOnView:weakSelf.showView caption:nil image:nil acitivity:YES autoHideTime:0];
        }
...
- (void)buildVideoNavBar
{
    ...
    if (!self.stopButton) {
        ...
        [_totolTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.top.bottom.equalTo(weakSelf.navBar);
...
//        _currentTimeLabel.frame = CGRectMake(30, 0, 52, 44);
        ...
        [_currentTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(weakSelf.stopButton.mas_right).offset(0);
            make.centerY.equalTo(weakSelf.navBar);
            make.width.mas_equalTo(52);
        }];
    }
    
    //单曲循环播放
    if (!self.sigleCircleButton) {
        self.sigleCircleButton = [UIButton buttonWithType:UIButtonTypeCustom];
        //        sigleCircleButton.frame = CGRectMake(0, 0, 44, 44);
        [_sigleCircleButton addTarget:self action:@selector(resumeOrPause) forControlEvents:UIControlEventTouchUpInside];
        [_sigleCircleButton setImage:[self imageNamed:@"icon_pause"] forState:UIControlStateNormal];
        [_sigleCircleButton setImage:[self imageNamed:@"icon_pause_hl"] forState:UIControlStateHighlighted];
        [_navBar addSubview:_sigleCircleButton];
        [_sigleCircleButton mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.top.bottom.equalTo(weakSelf.navBar);
            make.width.mas_equalTo(44);
        }];
    }
...
//        _totolTimeLabel.frame = CGRectMake(kScreenWidth-52-15, 0, 52, 44);
        ...
        [_totolTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            make.right.equalTo(weakSelf.sigleCircleButton.mas_left).offset(0);
            make.centerY.equalTo(weakSelf.navBar);
            make.width.mas_equalTo(52);
        }];
    }
    ...
    if (!self.videoProgressView) {
        ...
        _videoProgressView.trackTintColor = [UIColor lightGrayColor];   // 未填充部分颜色
//        _videoProgressView.frame = CGRectMake(62+30, 21, kScreenWidth-124-44, 20);
        ...
        [_videoProgressView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(weakSelf.navBar).offset(44+10+52);
            make.right.equalTo(weakSelf.navBar).offset(-(44+10+52));
            make.centerY.equalTo(weakSelf.navBar);
    ...
    if (!self.playSlider) {
        ...
        self.playSlider = [[TBSlider alloc] init];
//        _playSlider.frame = CGRectMake(62+30, 0, kScreenWidth-124-44, 44);
        ...
        _playSlider.minimumTrackTintColor = [UIColor redColor];
...
- (void)showOrHideTabView:(BOOL)show
{
    self.navBar.hidden = !show;
    self.navBar.alpha = show?0:1;
    [UIView animateWithDuration:1 animations:^{
        self.navBar.alpha = show?1:0;
    }];
}
@end
